...
class Agent(object):
    ...
    def policy_forward_pass(self, sy_ob_no):
        ...
        if self.discrete:
            ...
            # sy_logstd = tf.get_variable(name="log_std", initializer=np.ones(self.ac_dim, dtype=np.float32), dtype=tf.float32, trainable=True)
            sy_logstd = tf.get_variable("sy_log_std", [self.ac_dim], dtype=tf.float32, trainable=True)
    ...
    def sample_action(self, policy_parameters):
        ...
        if self.discrete:
            ...
            sy_sampled_ac = tf.random_normal([1, self.ac_dim]) * sy_std + sy_mean
    ...
    def get_log_prob(self, policy_parameters, sy_ac_na):
        ...
        if self.discrete:
            ...
            sy_mean, sy_logstd = policy_parameters
            # YOUR_HW2 CODE_HERE
            EPS = 1e-8 # avoid log of 0
            sy_std = tf.exp(sy_logstd)
            sy_logprob_n = -0.5 * (((sy_ac_na - sy_mean) / (sy_std + EPS)) ** 2 + 2 * sy_logstd + np.log(2 * np.pi))
    ...
    def sample_trajectory(self, env, animate_this_episode):
        ...
        while True:
            if animate_this_episode:
                ...
                time.sleep(0.1)
            obs.append(ob)
            ac = self.sess.run(self.sy_sampled_ac, feed_dict={self.sy_ob_no: ob.reshape([1, self.ob_dim])}) # YOUR HW2 CODE HERE
            ac = ac[0]
            acs.append(ac)
    ...
    def estimate_advantage(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        # YOUR CODE HERE
        q_n = re_n + self.gamma * (1 - terminal_n) * self.sess.run(self.critic_prediction, feed_dict={self.sy_ob_no: next_ob_no}) # batch
        v_n = self.sess.run(self.critic_prediction, feed_dict={self.sy_ob_no: ob_no})
        adv_n = q_n - v_n

        if self.normalize_advantages:
            EPS = 1e-8
            adv_n = (adv_n - np.mean(adv_n)) / (np.std(adv_n) + EPS) # YOUR_HW2 CODE_HERE
        return adv_n
    ...
    def update_critic(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        for _ in range(self.num_target_updates):
            target_n = re_n + self.gamma * (1 - terminal_n) * self.sess.run(self.critic_prediction, feed_dict={self.sy_ob_no: next_ob_no}) # batch
            for _ in range(self.num_grad_steps_per_target_update):
                self.sess.run([self.critic_update_op, self.critic_loss], feed_dict={self.sy_ob_no: ob_no, self.sy_target_n: target_n})
