...
class Histogram(Density_Model):
    ...
    def get_prob(self, states):
        ...
        raw_probs = counts / float(self.total)
        return probs
...
class Exemplar(Density_Model):
    ...
    def make_encoder(self, state, z_size, scope, n_layers, hid_size):
        ...
        z_logstd = tf.get_variable("z_logstd", initializer=tf.zeros(z_size), dtype=tf.float32, trainable=True)
    ...
    def get_likelihood(self, state1, state2):
        ...
        likelihood = self.sess.run(self.likelihood, feed_dict={self.state1: state1, self.state2: state2, self.discrim_target: np.ones([state1.shape[0], 1])})
        return likelihood
